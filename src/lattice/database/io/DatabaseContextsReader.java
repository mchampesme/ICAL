...
public class DatabaseContextsReader extends DatabaseAbstractReader {
	...
	public DatabaseContextsReader(DatabaseManagement dbm, Vector relations, String ctxName) {
		this.dbm = dbm;
	...
	public RelationalContextFamily readRelationalContext() throws BadInputDataException, IOException { 
		...
		RelationalContextFamily relCtxFam = new RelationalContextFamily(getDefaultNameForData());
		...
		for (int i=0; i<relations.size(); i++) {
			...
			else if ( type.equals(DatabaseFunctions.INTER_OBJECT_TYPE) ) {
				relCtxFam.add(readInterObjectBinaryRelation(relName));
			}
	 ...
	 * @throws IOException
	 */
	public MatrixBinaryRelationBuilder readBinaryRelation(String relName) throws BadInputDataException,IOException {
		...
		MatrixBinaryRelationBuilder binRel = new MatrixBinaryRelationBuilder(nbObj, nbAtt);
	 ...
	 * @throws IOException
	 */
	public InterObjectBinaryRelation readInterObjectBinaryRelation(String relName) throws BadInputDataException,IOException {
		
		String objectsContextName = dbm.getIORObjectsContextName(relName);
		String attributesContextName = dbm.getIORAttributesContextName(relName);

		Vector obj = dbm.getObjects(objectsContextName);
		Vector att = dbm.getObjects(attributesContextName);
		int nbObj = obj.size();
		int nbAtt = att.size();

		InterObjectBinaryRelation iobRel = new InterObjectBinaryRelation(nbObj, nbAtt);
		iobRel.setName(relName);
		iobRel.setObjectsContextName(objectsContextName);
		iobRel.setAttributesContextName(attributesContextName);
		
		for (int i=0; i<nbAtt; i++) {
			iobRel.replaceAttribute(i, new DefaultFormalAttribute(att.get(i).toString()));
		}
		
		for (int i=0; i<nbObj; i++) {

			iobRel.replaceObject(i, (FormalObject)obj.get(i));
			Vector rel = dbm.getInterObjectBinaryRelation(relName, obj.get(i).toString());

			for (int j=0; j<nbAtt; j++) {

				iobRel.setRelation(i, j, false);
				String attribute = att.get(j).toString();

				for (int k=0; k<rel.size(); k++) {

					if (attribute.equals(rel.get(k).toString())) {
						iobRel.setRelation(i, j, true);
					}
				}
			}
		}
	 ...
	 * @throws IOException
	 */
	public ScalingBinaryRelation readScalingBinaryRelation(String relName) throws BadInputDataException,IOException {
