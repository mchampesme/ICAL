...
public class Gagci_NoInc_Param extends JDialog implements ActionListener, ListSelectionListener {
	...
	private Vector lesInterObjectBinRel=null;
	...
	private Gagci_NoInc_Param(RelationalContextFamily relCtx) {
		...
		for(int i=0;i<relCtx.size();i++){
			if( relCtx.get(i) instanceof MatrixBinaryRelationBuilder
			    && !(relCtx.get(i) instanceof InterObjectBinaryRelation)) 
			    lesBinRel.add(relCtx.get(i));
		}
		allBinaryRelations=new JList(lesBinRel);
		allBinaryRelations.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		allBinaryRelations.addListSelectionListener(this);

		lesInterObjectBinRel=new Vector();
		for(int i=0;i<relCtx.size();i++){
			if( relCtx.get(i) instanceof InterObjectBinaryRelation) lesInterObjectBinRel.add(relCtx.get(i));
		}
		allInterObjectBinaryRelations=new JList(lesInterObjectBinRel);
		allInterObjectBinaryRelations.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
	...
	private void initKiSelectionPanel(){
		...
		JScrollPane scp = new JScrollPane(allBinaryRelations, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	private void initRelationSelectionPanel(){
		...
		for(int i=0;i<setOfKi.size();i++) setOfKiRelation.add(new Hashtable());

		JScrollPane scp1 = new JScrollPane(listOfKi1, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		JScrollPane scp2 = new JScrollPane(listOfKi2, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		JScrollPane scp3 = new JScrollPane(allInterObjectBinaryRelations, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	private void initGraphPanel(){
		...
		JScrollPane scp1 = new JScrollPane(lesKiFin, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		JScrollPane scp2 = new JScrollPane(shgGraph, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void valueChanged(ListSelectionEvent arg0) {
		...
		if(arg0.getSource()==listOfKi1 || arg0.getSource()==listOfKi2){
			if(listOfKi1.getSelectedIndex()!=-1 && listOfKi2.getSelectedIndex()!=-1){
				MatrixBinaryRelationBuilder theRel1=(MatrixBinaryRelationBuilder)listOfKi1.getSelectedValue();
				MatrixBinaryRelationBuilder theRel2=(MatrixBinaryRelationBuilder)listOfKi2.getSelectedValue();
				Vector theRels=(Vector)((Hashtable)setOfKiRelation.elementAt(listOfKi1.getSelectedIndex())).get(theRel2.getName());
		...
		if(arg0.getSource()==allInterObjectBinaryRelations){
			if(listOfKi1.getSelectedIndex()!=-1 && listOfKi2.getSelectedIndex()!=-1){
				...
				//System.out.println("Taille theRels="+theRels.size());
				((Hashtable)setOfKiRelation.elementAt(listOfKi1.getSelectedIndex())).put(theRel2.getName(),theRels);
			} else {
				if(allInterObjectBinaryRelations.getSelectedIndex()!=-1) allInterObjectBinaryRelations.clearSelection();
			}
		}
		...
		if(arg0.getSource()==shgGraph){
			if(lesKiFin.getSelectedIndex()==-1) shgGraph.clearSelection();
			else {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource()==nextButton && getContentPane()==kiRelationSeletionPanel){
			if(setOfKi.size()>0){
				Status=NEXT;
				setVisible(false);
				initGraphPanel();
		...
		if((arg0.getSource()==nextButton && getContentPane()==kiSeletionPanel) || (arg0.getSource()==backButton && getContentPane()==initGraphPanel)){
			if(setOfKi.size()>0){
				Status=NEXT;
				setVisible(false);
		...
		if(arg0.getSource()==terminatedButton){
			for(Enumeration iter=graphSelection.keys();iter.hasMoreElements();){
				String Ki_name=(String)iter.nextElement();
				lesGraphInit.put(Ki_name,((RelationBuilder)lesGraph.elementAt(((Integer)graphSelection.get(Ki_name)).intValue())).getLattice());
			}
			Status=TERMINATED;
			dispose();
		}
	...
	public static boolean askParameter(RelationalContextFamily relCtx, RelationalContextEditor controler){
