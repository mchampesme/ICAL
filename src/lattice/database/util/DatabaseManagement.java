...
public class DatabaseManagement {
	...
	/**
	 * A method that drops the database in the current running server, using a database name in parameter
	 */
	public void dropDatabase(String dbName) {
		...
		try {
			...
			System.err.println("The Database '" + dbName + "' cannot be dropped");
		}
	}
	
	
	/**
	 * A method that creates the database in the current running server, using a database name in parameter
	 */
	public void createDatabase(String dbName) {
		...
		try {
			...
			System.err.println("The Database '" + dbName + "' cannot be created");
		}
	}
	...
	public void deleteRelations(Vector relations) {
		...
		for (int i=0; i<relations.size(); i++) {
			deleteRelation(DatabaseFunctions.filter(relations.get(i).toString()))	;
		}
	}
		...
		if (!relType.equals(DatabaseFunctions.UNRECOGNIZED_TYPE)) {
			...
			try {
				...
				for (int i=0; i<derived.size(); i++) {
					...
					if (derivedType.equals(DatabaseFunctions.SCALING_BINARY_TYPE)) {
						if (DatabaseFunctions.showConfirmDialog(
								"Also deletes the '" + derivedName +
								"' scaling relation derived from '" + relName + "'?")) {
							stat.executeUpdate(deleteDerived);
							deleteRelation(derivedName);
						}
					} else if (!derivedType.equals(DatabaseFunctions.UNRECOGNIZED_TYPE)) {
						stat.executeUpdate(deleteDerived);
						deleteRelation(derivedName);
					}
				}
				...
				if (!parents.isEmpty()) {	
					if (DatabaseFunctions.showConfirmDialog(
							"Also delete the contexts from which '" + relName + "' derives?")) {
				...
				if (!relType.equals(DatabaseFunctions.INTER_OBJECT_TYPE)) {
					...
					stat.executeUpdate(dropTableGeneric + relName + OBJ_TABLE_SUFFIX);
					stat.executeUpdate(dropTableGeneric + relName + ATT_TABLE_SUFFIX);
				}
	...
	/**
	 * A method to create a RCF Schema (creates the tables and then adds the relations)
	 */
	public void createRCFSchema(Vector relations, RelationalContextFamily relCtxFam) {
		...
		String dropDerivationsTable = "DROP TABLE IF EXISTS " + RCF_RELATIONS_DEPENDENCES;
		String createDerivationsTable = "CREATE TABLE " + RCF_RELATIONS_DEPENDENCES + " (" +
		RELATION_NAME + " VARCHAR(30) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + " ON DELETE CASCADE, " +
		PARENT_RELATION + " VARCHAR(30) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + " ON DELETE CASCADE, PRIMARY KEY (" +
	 ...
	 */
	public void addRelationsToRCFSchema(Vector relations, RelationalContextFamily relCtxFam) {
		
		String insertGeneric = "INSERT INTO " + RCF_RELATIONS_PROPERTIES + " VALUES (";
		...
		try {
			...
			for (int i=0; i<relations.size(); i++) {
				...
				if (absRel instanceof ScalingBinaryRelation) {
					...
					String message = "Please choose the parent relation of '" + relName + "'";
					
					addToDependencesTable(
							relName,
							DatabaseFunctions.showInputDialog(
									message,
									getRelationsOfType(
											relCtxFam,
				...
				else if (absRel instanceof InterObjectBinaryRelation) {
					insert += "'" + DatabaseFunctions.INTER_OBJECT_TYPE + "', '" +
					((InterObjectBinaryRelation)absRel).getObjectsContextName() +
					OBJ_TABLE_SUFFIX + "', '" +
					((InterObjectBinaryRelation)absRel).getAttributesContextName()
					+ OBJ_TABLE_SUFFIX + "')";
					stat.executeUpdate(insert);
					String objName = ((InterObjectBinaryRelation)absRel).getObjectsContextName();
					String attName = ((InterObjectBinaryRelation)absRel).getAttributesContextName();
					addToDependencesTable(relName, objName);
					if (!objName.equals(attName)) {
						addToDependencesTable(relName, attName);
					}
					createInterObjectBinaryTable(relName, (InterObjectBinaryRelation) absRel);
				}
					...
					relName + ATT_TABLE_SUFFIX + "')";
					stat.executeUpdate(insert);
					createBinaryTable(relName, (MatrixBinaryRelationBuilder) absRel); 
				}
			}
		} catch (Exception e) {
	 ...
	 */
	public void createBinaryTable(String tableName, MatrixBinaryRelationBuilder binRel) {
		
		String objTable = tableName + OBJ_TABLE_SUFFIX;
		String attTable = tableName + ATT_TABLE_SUFFIX;
		
		String createObjTable = "CREATE TABLE " + objTable + " (" +
		OBJ_COLUMN + " VARCHAR(30) PRIMARY KEY)";
		String createAttTable = "CREATE TABLE " + attTable + " (" +
		ATT_COLUMN + " VARCHAR(30) PRIMARY KEY)";
		String createBinaryTable = "CREATE TABLE " + tableName + " (" +
		OBJ_COLUMN +
		" VARCHAR(30) NOT NULL REFERENCES " + objTable +
		" ON DELETE CASCADE, " + ATT_COLUMN +
		" VARCHAR(30) NOT NULL REFERENCES " + attTable +
		...
		try {
			...
			for (int i=0; i<obj.size(); i++) {
				stat.executeUpdate(insertObj + filterObjName(obj.get(i).toString()) + "')");
			}
			
			for (int j=0; j<att.size(); j++) {
				stat.executeUpdate(insertAtt + filterAttName(att.get(j).toString()) + "')");
			}
			...
			for (int i=0; i<obj.size(); i++) {
				for (int j=0; j<att.size(); j++) {
					if (binRel.getRelation(i,j).toString().equals("X")) {
						...
						stat.executeUpdate(	insertBinary + filterObjName(obj.get(i).toString()) + "', '" +
	...
	/**
	 * A method to create tables and fill an inter-object binary relation in the database 
	 */
	public void createInterObjectBinaryTable(String tableName, InterObjectBinaryRelation interObjRel) {
		
		String objTable = interObjRel.getObjectsContextName() + OBJ_TABLE_SUFFIX;
		String attTable = interObjRel.getAttributesContextName() + OBJ_TABLE_SUFFIX;
		
		String createTable = "CREATE TABLE " + tableName + " (" +
		IOR_OBJ_COLUMN +
		" VARCHAR(30) NOT NULL REFERENCES " + objTable +
		" ON DELETE CASCADE, " + IOR_ATT_COLUMN +
		" VARCHAR(30) NOT NULL REFERENCES " + attTable +
		" ON DELETE CASCADE, PRIMARY KEY (" +
		IOR_OBJ_COLUMN + ", " +
		IOR_ATT_COLUMN + "))";
		
		String insertGeneric = "INSERT INTO " + tableName + " VALUES ('";
		
		List<FormalObject> obj = interObjRel.getObjects();
		List<FormalAttribute> att = interObjRel.getAttributes();
		...
		try {
			...
			for (int i=0; i<obj.size(); i++) {
				for (int j=0; j<att.size(); j++) {
					if (interObjRel.getRelation(i,j).toString().equals("X")) {
						String insert = insertGeneric +
	...
	public void addToDependencesTable(String tableName, String parentRelation) {
		...
		String insertDependences = "INSERT INTO " + RCF_RELATIONS_DEPENDENCES + " VALUES ('" +
		tableName + "', '" + parentRelation + "')";
		try {
			stat = dbc.getConnection().createStatement();
	...
	/**
	 * A method to get the inter-object relation objects context name from its table name
	 */
	public String getIORObjectsContextName(String tableName) {
		String result = null;
		try {
			stat = dbc.getConnection().createStatement();
			String query = "SELECT " + OBJECTS_TABLE +
			" FROM " + RCF_RELATIONS_PROPERTIES +
			" WHERE " + RELATION_NAME +
			" = '" + tableName + "'";
			...
			result = result.substring(0,
					result.length()-OBJ_TABLE_SUFFIX.length());
	...
	/**
	 * A method to get the inter-object relation attributes context name from its table name
	 */
	public String getIORAttributesContextName(String tableName) {
		String result = null;
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + ATTRIBUTES_TABLE +
					" FROM " + RCF_RELATIONS_PROPERTIES +
					" WHERE " + RELATION_NAME +
					" = '" + tableName + "'");
			rs.next();
	...
	public Vector getBinaryRelation(String tableName, String object) {
		...
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + ATT_COLUMN +
	...
	/**
	 * A method to get the inter-object binary relation between an object and its attributes
	 * @return a <code>Vector</code> that contains the attributes of an object
	 */
	public Vector getInterObjectBinaryRelation(String tableName, String object) {
		...
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + IOR_ATT_COLUMN +
					" FROM " + tableName + " WHERE " +
					IOR_OBJ_COLUMN + " = '" + object + "'");
			while (rs.next()) {
				result.addElement(rs.getString(1));
			}
		} catch (Exception e) {
	 ...
	 * @return a <code>Vector</code> that contains the values of an object for a specified attribute
	 */
	public Vector getValuedRelation(String tableName, String object, String attribute) {
		Vector result = new Vector();
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + VAL_COLUMN +
					" FROM " + tableName +
	...
	public Vector getTables() {
		...
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + RELATION_NAME +
					" FROM " + RCF_RELATIONS_PROPERTIES);
			while (rs.next()) {
	...
	public String getRelationType(String tableName) {
		...
		String query = "SELECT " + TABLE_TYPE +
		" FROM " + RCF_RELATIONS_PROPERTIES +
		" WHERE " + RELATION_NAME + " = '" + tableName + "'"; 
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(query);
			rs.first();
	...
	public Vector getParentContexts(String relName) {
		Vector result = new Vector();
		String query = "SELECT " + PARENT_RELATION + " FROM " + RCF_RELATIONS_DEPENDENCES +
		" WHERE " + RELATION_NAME + " = '" + relName + "'";
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(query);
			...
			System.err.println("Error getting the parent contexts of '" + relName + "'");
		}
		return result;
	}
	...
	public Vector getDerivedContexts(String relName) {
		Vector result = new Vector();
		String query = "SELECT " + RELATION_NAME + " FROM " + RCF_RELATIONS_DEPENDENCES +
		" WHERE " + PARENT_RELATION + " = '" + relName + "'";
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(query);
			...
			System.err.println("Error getting the contexts derived from '" + relName + "'");
		}
		return result;
	}
	 ...
	 */
	public String[] getRelationsOfType(RelationalContextFamily relCtxFam, String type) {
	...
	public boolean isRelationExisting(String relName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + RELATION_NAME + " FROM " + RCF_RELATIONS_PROPERTIES +
					" WHERE " + RELATION_NAME + " = '" + relName + "'");
			rs.first();
			if (rs.last()) {
				return true;
	...
	protected void createRCFRulesBasisSchema() {
		...
		String dropPropertiesTable = "DROP TABLE IF EXISTS " + RCF_RULES_BASIS_PROPERTIES;
		String createPropertiesTable = "CREATE TABLE " + RCF_RULES_BASIS_PROPERTIES + " (" +
		RULES_BASIS_ID + " INT(5) PRIMARY KEY AUTO_INCREMENT, " +
		DATASET_NAME + " VARCHAR(30) NOT NULL, " +
		RELATION_NAME + " VARCHAR(30) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + "." + RELATION_NAME + " ON DELETE CASCADE, " +
		MIN_SUPPORT + " DECIMAL(3,2) NOT NULL, " +
		...
		RULES_BASIS_ID + " INT(5) NOT NULL REFERENCES " +
		RCF_RULES_BASIS_PROPERTIES + " ON DELETE CASCADE, " +
		PREMISE + " VARCHAR(200) NOT NULL, " +
		CONSEQUENCE + " VARCHAR(200) NOT NULL, " +
		SUPPORT + " DECIMAL(3,2), " +
		CONFIDENCE + " DECIMAL(3,2), " +
		"PRIMARY KEY (" + RULES_BASIS_ID + ", " + PREMISE + ", " + CONSEQUENCE + "))";
	 ...
	 */
	public void addRulesBasis(String dataset, String relName, Set<Rule> rules, double minSupport, double minConfidence) {
		
		String insertProperties =	"INSERT INTO " + RCF_RULES_BASIS_PROPERTIES +
		" (" + DATASET_NAME + ", " + RELATION_NAME + ", " +
		MIN_SUPPORT + ", " + MIN_CONFIDENCE + ") VALUES ('" +
		dataset + "', '" + relName + "', " + minSupport + ", " + minConfidence + ")";
		
		String insertRuleGeneric = "INSERT INTO " + RCF_RULES_BASIS + " VALUES (LAST_INSERT_ID(), ";
		...
		try {
			...
			while ( rulesIterator.hasNext()) {
				...
				Iterator<FormalAttribute> premiseIterator = currentRule.getAntecedent().iterator();
				Iterator<FormalAttribute> consequenceIterator = currentRule.getConsequence().iterator();
				
				String premises = "";
				if (premiseIterator.hasNext()) {
					premises += filterAttName(((Object)premiseIterator.next()).toString());
				}
				while ( premiseIterator.hasNext() ) {
					premises += SEPARATOR + filterAttName(((Object)premiseIterator.next()).toString());
				}
				insertRule += "'" + premises + "', ";
				
				String consequences = "";
				if (consequenceIterator.hasNext()) {
					consequences += filterAttName(((Object)consequenceIterator.next()).toString());
				}
				while ( consequenceIterator.hasNext() ) {
					consequences += SEPARATOR + filterAttName(((Object)consequenceIterator.next()).toString());
				}
				insertRule +=
					"'" + consequences + "', " +
					Double.toString(((double)((int)(currentRule.getSupport()*100.0)))/100.0) +
					", " +
					Double.toString(((double)((int)(currentRule.getConfiance()*100.0)))/100.0) +
					")";
	...
	public void deleteRulesBasis(String rulesBasisID) {
		...
		try {
			...
			stat.executeUpdate(	"DELETE FROM " + RCF_RULES_BASIS_PROPERTIES + " WHERE " +
					RULES_BASIS_ID + " = " + rulesBasisID);
	...
	/**
	 * A method to get the list of rules basis IDs associated to a certain relation from the database
	 */
	public Vector getRulesBasisIDsOfRelation(String relName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT " + RULES_BASIS_ID + " FROM " + RCF_RULES_BASIS_PROPERTIES +
					" WHERE " + RELATION_NAME + " = '" + relName + "'");
			Vector result = new Vector();
			while (rs.next()) {
				result.addElement(rs.getString(1));
			...
			System.err.println("Error getting the rules basis related to '" + relName + "'");
		}
		return null;
	}
	...
	public Vector getAllRulesBasisList() {
		...
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT " + RULES_BASIS_ID + ", " + DATASET_NAME + ", " +
					RELATION_NAME + ", " + MIN_SUPPORT + ", " + MIN_CONFIDENCE +
					" FROM " + RCF_RULES_BASIS_PROPERTIES);
			Vector rulesBasis = new Vector();
			while (rs.next()) {
				rulesBasis.addElement(	"ID #" + rs.getString(1) + " [" + rs.getString(2) + ", " +
	...
	/**
	 * A method to get the list of rules basis associated to a certain relation from the database
	 */
	public Vector getRelationRulesBasisList(String relName) {
		...
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT " + RULES_BASIS_ID + ", " + DATASET_NAME + ", " +
					MIN_SUPPORT + ", " + MIN_CONFIDENCE + " FROM " + RCF_RULES_BASIS_PROPERTIES +
					" WHERE " + RELATION_NAME + " = '" + relName + "'");
			Vector rulesBasis = new Vector();
			while (rs.next()) {
				rulesBasis.addElement(	"ID #" + rs.getString(1) + " [" + rs.getString(2) + ", " + relName +
	...
	public Set<Rule> getRules(String rulesBasisID) {
		try {
			...
			rs = stat.executeQuery(	"SELECT " + PREMISE + ", " + CONSEQUENCE + ", " +
					SUPPORT + ", " + CONFIDENCE + " FROM " + RCF_RULES_BASIS +
					" WHERE " + RULES_BASIS_ID + " = " + rulesBasisID);
			...
			while (rs.next()) {
				...
				StringTokenizer premiseTokenizer = new StringTokenizer(
						rs.getString(1), SEPARATOR);
				StringTokenizer consequenceTokenizer = new StringTokenizer(
						rs.getString(2), SEPARATOR);
				
				while (premiseTokenizer.hasMoreTokens()) {
					premise.add(new DefaultFormalAttribute(premiseTokenizer.nextToken()));
				}
				
				while (consequenceTokenizer.hasMoreTokens()) {
					consequence.add(new DefaultFormalAttribute(consequenceTokenizer.nextToken()));
				}
	...
	public String getDatasetName(String rulesBasisID) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT "  + DATASET_NAME + " FROM " + RCF_RULES_BASIS_PROPERTIES +
					" WHERE " + RULES_BASIS_ID + " = " + rulesBasisID);
			rs.first();
			if (rs.last()) {
				return rs.getString(1);
			...
			System.err.println("Error getting the dataset name of rules basis '" + rulesBasisID + "'");
		}
		return null;
	}
	...
	public double getMinConfidence(String rulesBasisID) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT "  + MIN_CONFIDENCE + " FROM " + RCF_RULES_BASIS_PROPERTIES +
					" WHERE " + RULES_BASIS_ID + " = " + rulesBasisID);
			rs.first();
			if (rs.last()) {
			...
			System.err.println("Error getting the minimal confidence of rules basis '" + rulesBasisID + "'");
		}
		return 0.0;
	}
	...
	public double getMinSupport(String rulesBasisID) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery("SELECT "  + MIN_SUPPORT + " FROM " + RCF_RULES_BASIS_PROPERTIES +
					" WHERE " + RULES_BASIS_ID + " = " + rulesBasisID);
			...
			System.err.println("Error getting the minimal support of rules basis '" + rulesBasisID + "'");
		}
		return 0.0;
	}
	...
	/**
	 * A method to create the Lattices Relational Schema in the database (creates the tables)
	 */
	protected void createRCFLatticesSchema() {
		
		String dropLatticesProperties = "DROP TABLE IF EXISTS " + RCF_LATTICES_PROPERTIES;
		String createLatticesProperties =	"CREATE TABLE " + RCF_LATTICES_PROPERTIES + " (" +
		LATTICE_ID + " INT(5) PRIMARY KEY AUTO_INCREMENT, " +
		RELATION_NAME + " VARCHAR(30) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + "." + RELATION_NAME + " ON DELETE CASCADE, " +
		LATTICE_DESCRIPTION + " VARCHAR(200), " +
		LATTICE_TYPE + " VARCHAR(30), " +
		MIN_SUPPORT + " DECIMAL(3,2), " +
		...
		LATTICE_ID + " INT(5) NOT NULL REFERENCES " +
		RCF_LATTICES_PROPERTIES + " ON DELETE CASCADE, " +
		LATTICE_NODE + " INT(5) NOT NULL, " +
		" PRIMARY KEY (" + LATTICE_ID + ", " + LATTICE_NODE + "))";
		
		String dropLatticesParents = "DROP TABLE IF EXISTS " + RCF_LATTICES_PARENTS;
		String createLatticesParents = 	"CREATE TABLE " + RCF_LATTICES_PARENTS + " (" +
		LATTICE_ID + " INT(5) NOT NULL REFERENCES " +
		RCF_LATTICES_PROPERTIES + " ON DELETE CASCADE, " +
		LATTICE_NODE + " INT(5) NOT NULL, " +
		PARENT_NODE + " INT(5) NOT NULL, " +
		" PRIMARY KEY (" + LATTICE_ID + ", " + LATTICE_NODE +
		", " + PARENT_NODE + "))";
		
		String dropLatticesExtents = "DROP TABLE IF EXISTS " + RCF_LATTICES_EXTENTS;
		String createLatticesExtents = 	"CREATE TABLE " + RCF_LATTICES_EXTENTS + " (" +
		LATTICE_ID + " INT(5) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + "." + RELATION_NAME + " ON DELETE CASCADE, " +
		LATTICE_NODE + " INT(5) NOT NULL, " +
		LATTICE_OBJECT + " VARCHAR(30) NOT NULL, " +
		"PRIMARY KEY (" + LATTICE_ID + ", " + LATTICE_NODE +
		", " + LATTICE_OBJECT + "))";
		
		String dropLatticesIntents = "DROP TABLE IF EXISTS " + RCF_LATTICES_INTENTS;
		String createLatticesIntents = 	"CREATE TABLE " + RCF_LATTICES_INTENTS + " (" +
		LATTICE_ID + " INT(5) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + "." + RELATION_NAME + " ON DELETE CASCADE, " +
		LATTICE_NODE + " INT(5) NOT NULL, " +
		LATTICE_ATTRIBUTE + " VARCHAR(30) NOT NULL, " +
		"PRIMARY KEY (" + LATTICE_ID + ", " + LATTICE_NODE +
		", " + LATTICE_ATTRIBUTE + "))";
		
		String dropLatticesGenerators = "DROP TABLE IF EXISTS " + RCF_LATTICES_GENERATORS;
		String createLatticesGenerators = 	"CREATE TABLE " + RCF_LATTICES_GENERATORS + " (" +
		LATTICE_ID + " INT(5) NOT NULL REFERENCES " +
		RCF_RELATIONS_PROPERTIES + "." + RELATION_NAME + " ON DELETE CASCADE, " +
		LATTICE_NODE + " INT(5) NOT NULL, " +
		LATTICE_GENERATOR + " INT(5) NOT NULL, " +
		LATTICE_ATTRIBUTE + " VARCHAR(30) NOT NULL, " +
		"PRIMARY KEY (" + LATTICE_ID + ", " + LATTICE_NODE +
		", " + LATTICE_GENERATOR + ", " + LATTICE_ATTRIBUTE + "))";
	...
	/**
	 * A method to save in the databsae the currently associated lattice of a relation
	 */
	public void saveLattice(RelationBuilder absRel) {
		...
		try {
			...
			RELATION_NAME + ", " + LATTICE_DESCRIPTION + ", " +
			LATTICE_TYPE + ", " + MIN_SUPPORT + ", " + LATTICE_TOP_NODE +
			", " + LATTICE_BOTTOM_NODE + ") VALUES ('" + relName +
			"', '" + description + "', '" + type + "', " + minSupport +
			", " + topNode + ", " + bottomNode + ")";
			
			stat.executeUpdate(insertProperties);
			
			String insertNodeGeneric = "INSERT INTO " + RCF_LATTICES_NODES + " VALUES (LAST_INSERT_ID(), ";
			String insertParentGeneric = "INSERT INTO " + RCF_LATTICES_PARENTS + " VALUES (LAST_INSERT_ID(), "; 
			String insertExtentGeneric = "INSERT INTO " + RCF_LATTICES_EXTENTS + " VALUES (LAST_INSERT_ID(), ";
			String insertIntentGeneric = "INSERT INTO " + RCF_LATTICES_INTENTS + " VALUES (LAST_INSERT_ID(), ";
			String insertGeneratorGeneric = "INSERT INTO " + RCF_LATTICES_GENERATORS + " VALUES (LAST_INSERT_ID(), ";
			...
			while (Q.size() != 0) {
				...
				// Extent
				for (Iterator it = nodeToWrite.getContent().getExtent().iterator(); it.hasNext();) {
					String object = filterObjName(((FormalObject)it.next()).toString());
					stat.executeUpdate(insertExtentGeneric + latticeNode + ", '" + object + "')");
				}
				
				// Intent
				for (Iterator it = nodeToWrite.getContent().getIntent().iterator();	it.hasNext();) {
					String attribute = filterAttName(((FormalAttribute)it.next()).toString());
					stat.executeUpdate(insertIntentGeneric + latticeNode + ", '" + attribute + "')");
				}
				
				// Parents
				for (Iterator it = nodeToWrite.getParents().iterator(); it.hasNext();) {
					String parentNode = ((Integer)correspId.get(((ConceptNode)it.next()).getId())).toString();
					stat.executeUpdate(insertParentGeneric + latticeNode + ", " + parentNode + ")");
				}
				
				// Generators
				if (nodeToWrite.getContent().getGenerator().size() != 0) {
					int generator = 0;
					for (Iterator it = nodeToWrite.getContent().getGenerator().iterator(); it.hasNext();) {
						for (Iterator itG = ( (Intent) it.next()).iterator(); itG.hasNext();) {
							String attribute = filterAttName(((FormalAttribute)itG.next()).toString());
							stat.executeUpdate(	insertGeneratorGeneric + latticeNode + ", " +
									generator + ", '" + attribute + "')");
						}
						generator++;
					}
			...
			stat.executeUpdate(	"UPDATE " + RCF_LATTICES_PROPERTIES + " SET " + LATTICE_BOTTOM_NODE +
					" = " + latticeNode + " WHERE " + LATTICE_ID + " = LAST_INSERT_ID()");
		} catch (Exception e) {
	...
	public void deleteLattice(String latticeID) {
		...
		try {
			stat = dbc.getConnection().createStatement();
			stat.executeUpdate("DELETE FROM " + RCF_LATTICES_EXTENTS + " WHERE " + LATTICE_ID  + " = " + latticeID);
			stat.executeUpdate("DELETE FROM " + RCF_LATTICES_INTENTS + " WHERE " + LATTICE_ID  + " = " + latticeID);
			stat.executeUpdate("DELETE FROM " + RCF_LATTICES_GENERATORS + " WHERE " + LATTICE_ID  + " = " + latticeID);
			stat.executeUpdate("DELETE FROM " + RCF_LATTICES_NODES + " WHERE " + LATTICE_ID  + " = " + latticeID);
			stat.executeUpdate("DELETE FROM " + RCF_LATTICES_PARENTS + " WHERE " + LATTICE_ID  + " = " + latticeID);
			stat.executeUpdate("DELETE FROM " + RCF_LATTICES_PROPERTIES + " WHERE " + LATTICE_ID  + " = " + latticeID);
			
		} catch (Exception e) {
			System.err.println("Error deleting the lattice '" + latticeID + "'");
			e.printStackTrace();
		}
	}
	...
	public Vector getRelationLatticesList(String relName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT " + LATTICE_ID + ", " + LATTICE_DESCRIPTION +
					" FROM " + RCF_LATTICES_PROPERTIES +
					" WHERE " + RELATION_NAME + " = '" + relName + "'");
			Vector lattices = new Vector();
			while (rs.next()) {
				lattices.addElement("ID #" + rs.getString(1) + " [" + rs.getString(2) + "]");
			}
			return lattices;
		} catch (Exception e) {
			System.err.println("Error getting the lattices list for the relation '" + relName + "'");
			e.printStackTrace();
		}
		return null;
	}
	...
	protected String getLatticeProperty(String latticeID, String property) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT "  + property + " FROM " + RCF_LATTICES_PROPERTIES +
	...
	protected Vector getLatticeNodesList(String latticeID) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT "  + LATTICE_NODE + " FROM " + RCF_LATTICES_NODES +
					" WHERE " + LATTICE_ID + " = " + latticeID);
	...
	protected Vector getLatticeNodeExtent(String latticeID, String nodeName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT "  + LATTICE_OBJECT + " FROM " + RCF_LATTICES_EXTENTS +
					" WHERE " + LATTICE_ID + " = " + latticeID + " AND " +
	...
	protected Vector getLatticeNodeIntent(String latticeID, String nodeName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT "  + LATTICE_ATTRIBUTE + " FROM " + RCF_LATTICES_INTENTS +
					" WHERE " + LATTICE_ID + " = " + latticeID + " AND " +
					LATTICE_NODE + " = " + nodeName);
	...
	protected Vector getLatticeNodeParents(String latticeID, String nodeName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT "  + PARENT_NODE + " FROM " + RCF_LATTICES_PARENTS +
					" WHERE " + LATTICE_ID + " = " + latticeID + " AND " +
					LATTICE_NODE + " = " + nodeName);
	...
	protected Vector getLatticeNodeGenerators(String latticeID, String nodeName) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT " + LATTICE_GENERATOR + ", " + LATTICE_ATTRIBUTE +
					" FROM " + RCF_LATTICES_GENERATORS + " WHERE " + LATTICE_ID +
					" = " + latticeID + " AND " + LATTICE_NODE + " = " + nodeName);
	...
	protected String getTopNodeID(String latticeID) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT " + LATTICE_TOP_NODE + " FROM " + RCF_LATTICES_PROPERTIES +
					" WHERE " + LATTICE_ID + " = " + latticeID);
			rs.first();
			if (rs.last()) {
				return rs.getString(1);
	...
	protected String getBottomNodeID(String latticeID) {
		try {
			stat = dbc.getConnection().createStatement();
			rs = stat.executeQuery(	"SELECT " + LATTICE_BOTTOM_NODE + " FROM " + RCF_LATTICES_PROPERTIES +
					" WHERE " + LATTICE_ID + " = " + latticeID);
			rs.first();
			if (rs.last()) {
				return rs.getString(1);
	...
	public CompleteConceptLattice getLattice(String latticeID) {
		...
		double minSupport = (double) Double.parseDouble(getLatticeProperty(latticeID, MIN_SUPPORT));
		...
		for (int i=0; i<nodes.size(); i++) {
			...
			ConceptNode newNode = new ConceptNodeImp(new Integer(nodeName), new ConceptImp(extent, intent));
			...
			if (firstNode) {
				...
				for (int j=0; j<parentNodes.size(); j++) {
					ConceptNode supNode = (ConceptNode) processedNodes.get(Integer.parseInt((String)parentNodes.get(j)));
