...
/**
 * A Swing GUI to configure the lattices to delete from the database
 * @author Manuel Aupetit
 */
public class DeleteLatticesDialog extends JFrame {	private javax.swing.JPanel jContentPane = null;
	...
	private javax.swing.JScrollPane latticesScrollPane = null;
	...
	private javax.swing.JPanel latticesChoicePanel = null;
	
	
	/**
	 * Constructor using a database management parameter 
	 */
	public DeleteLatticesDialog(DatabaseManagement dbm) {
		super();
		this.dbm = dbm;
		initialize();
	}
	...
	private javax.swing.JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.add(getLatticesScrollPane(), java.awt.BorderLayout.CENTER);
	...
	/**
	 * This method initializes latticesScrollPane
	 * 
	 * @return javax.swing.JScrollPane
	 */
	private javax.swing.JScrollPane getLatticesScrollPane() {
		if(latticesScrollPane == null) {
			latticesScrollPane = new javax.swing.JScrollPane();
			latticesScrollPane.setViewportView(getLatticesChoicePanel());
		}
		return latticesScrollPane;
	}
	/**
		...
		if(connectButton == null) {
			...
			connectButton.addActionListener(new java.awt.event.ActionListener() { 
				public void actionPerformed(java.awt.event.ActionEvent e) {    
					DatabaseConfiguration dbc = new DatabaseConfiguration(dbm, DatabaseConfiguration.DELETE_LATTICES_MODE);
					dispose();
				}
			});
		}
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (int i=0; i<getCheckBoxes().length; i++) {
						if (getCheckBoxes()[i].isSelected()) {
							lattices.addElement(
									DatabaseFunctions.getLatticeIDFromDescription(
														getCheckBoxes()[i].getLabel()));
						}
					}
					DatabaseFunctions.deleteLattices(dbm, lattices);
					dbm.closeConnection();
					dispose();
				}
			});
	...
	private javax.swing.JLabel getInstructionsLabel() {
		if(instructionsLabel == null) {
			...
			instructionsLabel.setText("Select the lattices you want to delete from the selected database:");
	...
	private javax.swing.JPanel getLatticesChoicePanel() {
		if(latticesChoicePanel == null) {
			latticesChoicePanel = new javax.swing.JPanel();
			...
			latticesChoicePanel.setLayout(new java.awt.GridBagLayout());
			for (int i=0; i<getCheckBoxes().length; i++) {
				consGridBagConstraints9.gridy = i;
				latticesChoicePanel.add(getCheckBoxes()[i], consGridBagConstraints9);
			}
		}
		return latticesChoicePanel;
	}
	...
	private void refreshLattices() {
		Vector lattices = DatabaseFunctions.getAllLatticesList(dbm);
		this.checkBoxes = new JCheckBox[lattices.size()];
		for (int i=0; i<lattices.size(); i++) {
			checkBoxes[i] = new JCheckBox((String)lattices.get(i), true);
		}
	}
}
